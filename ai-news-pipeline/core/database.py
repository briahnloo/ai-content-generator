    ...
    def save(self, item: ContentItem):
        with self._conn() as conn:
            ...
            """, (
                ...
                item.status.value, item.ingested_at, item.author, item.published_at,
                item.script, item.video_url, item.video_path, item.youtube_id,
                item.content_score, score_breakdown_json, item.error, item.retry_count, datetime.utcnow()
            ))
